...
TEST_CASE("substitute_template replaces placeholders", "[utils]") {
    ...
    SECTION("empty data") {
        ...
        CHECK(substitute_template("{}", empty) == "");
        CHECK(substitute_template("{0}", empty) == "{0}");
    }

    SECTION("single element data") {
        std::vector<std::string> single = {"only"};
        CHECK(substitute_template("{}", single) == "only");
        CHECK(substitute_template("{0}", single) == "only");
    }

    SECTION("non-numeric placeholder kept as-is") {
        CHECK(substitute_template("{abc}", data) == "{abc}");
        CHECK(substitute_template("{0a}", data) == "{0a}");
    }

    SECTION("unclosed brace") {
        CHECK(substitute_template("{0 text", data) == "{0 text");
        CHECK(substitute_template("{ no close", data) == "{ no close");
    }
    ...
    SECTION("double-digit index") {
        ...
        large[10] = "ten";
        large[14] = "fourteen";
        CHECK(substitute_template("{10}", large) == "ten");
        CHECK(substitute_template("{14}", large) == "fourteen");
    }
}
